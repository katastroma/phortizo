...
func TestResolveFunc_EmptySecret(t *testing.T) {
	...
	resolve := credential.NewResolveFunc(&tests.MockCredentialReader{}, http.DefaultClient, newStore)
...
func TestResolveFunc(t *testing.T) {
	...
	newStore := func(_ string) object.Store[[]byte] { return tests.NewMockStore[[]byte]() }
	resolve := credential.NewResolveFunc(reader, http.DefaultClient, newStore)
