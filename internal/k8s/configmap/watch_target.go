...
func GetWatchTarget(ctx context.Context, client kubernetes.Interface, namespace, name string) (*source.Target, error) {
...
func ListWatchTargets(ctx context.Context, client kubernetes.Interface, namespace string) ([]*source.Target, error) {
	...
	targets := make([]*source.Target, 0, len(list.Items))
...
func watchTargetFromConfigMap(cm *corev1.ConfigMap) (*source.Target, error) {
	target, err := source.TargetFromConfigMap(cm.Data)
	if err != nil {
		return nil, err
	}

	target.Name = cm.Name
	target.CredentialSecret = cm.Annotations[source.CredentialSecretAnnotation]
	return &target, nil
}
...
func buildAnnotations(credentialSecret string) map[string]string {
	...
	return map[string]string{source.CredentialSecretAnnotation: credentialSecret}
}
